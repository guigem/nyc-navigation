...
from script.app.location_moderator import (
    verif_user_input,
    choose_right_network,
    compute_route,
    error_raiser,
)
...
def nav():
    ...
    if form.validate_on_submit():
        ...
        is_correct, message = error_raiser(
            entry_one=location_start, entry_two=location_to
        )
        if not is_correct:
...
def road(location_start: str, location_to: str, choice_user: str, choice_weight: str):
    ...
    print(
        "Datas passed : \n{} \n{} \n{} \n{}".format(
            location_start, location_to, choice_user, choice_weight
        )
    )
    ...
    folium_map = ox.folium.plot_route_folium(
        G,
        route=route,
    ...
    folium.Marker(
        location=start, popup="Start", icon=folium.Icon(color="green")
    ).add_to(folium_map)
    folium.Marker(location=end, popup="End", icon=folium.Icon(color="red")).add_to(
        folium_map
    )
    ...
    return render_template_string(
        ...
        div_placeholder=folium_map._repr_html_(),
    )
